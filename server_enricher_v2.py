...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('server_enricher_v2.log'),
...
class ServerEnricherV2:
    def __init__(self, db_path: str = "servers_v2.db"):
        self.db_path = db_path
        self.session: Optional[aiohttp.ClientSession] = None
        self.lunar_client_api_url = "https://api.github.com/repos/LunarClient/ServerMappings/contents/servers"
    ...
    async def fetch_lunar_client_servers(self) -> List[Dict[str, Any]]:
        """Fetch server data from LunarClient ServerMappings using GitHub API"""
        try:
            logger.info(f"Fetching server list from {self.lunar_client_api_url}")
            
            # First, get the list of server files
            async with self.session.get(self.lunar_client_api_url) as response:
                if response.status == 200:
                    server_files = await response.json()
                    logger.info(f"Found {len(server_files)} server files")
                    
                    # Fetch each server's data
                    servers = []
                    for server_file in server_files[:50]:  # Limit to first 50 for testing
                        server_name = server_file['name'].replace('.json', '')
                        server_data = await self.fetch_server_data(server_name)
                        if server_data:
                            servers.append(server_data)
                    
                    logger.info(f"Successfully fetched {len(servers)} servers from LunarClient")
                    return servers
                else:
                    logger.error(f"Failed to fetch server list: {response.status}")
                    return []
        except Exception as e:
            logger.error(f"Error fetching LunarClient data: {e}")
            return []
    
    async def fetch_server_data(self, server_name: str) -> Optional[Dict[str, Any]]:
        """Fetch individual server data from GitHub"""
        try:
            # Try to get the file content using GitHub API
            url = f"https://api.github.com/repos/LunarClient/ServerMappings/contents/servers/{server_name}.json"
            async with self.session.get(url) as response:
                if response.status == 200:
                    file_data = await response.json()
                    if 'content' in file_data and 'encoding' in file_data:
                        import base64
                    ...
                    logger.debug(f"Failed to fetch {server_name}: {response.status}")
                    return None
        except Exception as e:
            logger.debug(f"Error fetching {server_name}: {e}")
            return None
    ...
    async def process_servers(self, max_concurrent: int = 5):
    ...
    def export_to_json(self, output_file: str = "enriched_servers_v2.json"):
